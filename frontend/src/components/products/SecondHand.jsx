    ...
    const fetchCars = async () => {
      try {
        ...
        const filtered = allCars.filter(
          (car) =>
            car.category &&
            car.category.toLowerCase().replace(/\s/g, "") === "secondhand"
        );
  ...
  return (
    <section className="relative py-10 px-4 lg:px-0">
      <div className="container mx-auto text-center mb-10">
        <h2 className="text-2xl font-bold mb-4 capitalize">SecondHand Cars</h2>
      ...
      >
        {cars.length > 0 ? (
          ...
          <p className="text-center text-gray-500 w-full">No secondhand cars yet.</p>
        )}
      </div>
    </section>
  );
